// ScaleFX — pass 3 (porte de Sp00kyFox, MIT). Detecção de bordas níveis 1–6 e
// classificação de subpixels. Lê só o passe anterior (Source → uTex), numa
// vizinhança de até ±3 texels. Corpo do algoritmo preservado verbatim; só a
// amostragem foi adaptada. Param do preset baked: SFX_SCN=1.0.
...
vec4 effect(vec2 uv) {
    const float SFX_SCN = 1.0;
    vec2 ts = 1.0 / uInputSize;
#define TEX(X, Y) SAMPLE(uTex, uv + vec2(X, Y) * ts)
